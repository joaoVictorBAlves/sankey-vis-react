 ...
 */
export function generateDataset(numA, numQ, numK, percentage1 = null, percentage2 = null, percentage3 = null) {
    const nodes = [];
    const links = [];
 ...
 */
export function calculateNodePositions(nodeMap, K, gapA, gapQ, gapK, reductor_Q, reductor_K) {
 ...
 */
export function defineY0ForLinks(nodeMap, links, K, factor, reductor_Q, reductor_K) {
 ...
 */
export function defineY1ForLinks(nodeMap, links, K, factor, reductor_Q, reductor_K) {
    const nodeGroups = groupNodesByInitial(nodeMap); // Agrupar nós por inicial (A, Q, K)
 ...
 */
export function updateLinksAndNodesByLink(link, opacityValue, fillColor, Vs, As, type) {
    let conNodes = [];
    ...
    As._groups[0].filter(linkElement =>
        ...
        if (type == "over")
            d3.select(linkElement)
                ...
                .attr("opacity", opacityValue).raise();
        else if (type == "out")
            d3.select(linkElement)
                .attr("fill", () => {
 ...
 * @param {string} fillColor - A cor de preenchimento a ser aplicada aos nós conectados.
 */
export function updateLinksAndNodesByNode(node, opacityValue, fillColor, Vs, As, type) {
    let conLinks = [];
    let conNodes = [];
    ...
    const As = svg.selectAll(".link")
        ...
        .on("mouseover", function () {
            d3.select(this)
                .attr("opacity", 1.2)
                .attr("fill", () => {
                    if (d3.select(this)._groups[0][0].__data__.value == 1) return "#D06020";
                    if (d3.select(this)._groups[0][0].__data__.value == 2) return "#7150B0";
                    if (d3.select(this)._groups[0][0].__data__.value == 3) return "#58B0A0";
                    return "lightgray";
                }).raise();
            ...
            const linksAfter = targetNode.sourceLinks.filter(l => l.value == link.value)
            ...
            linksAfter.forEach(afterLink => {
                svg.append("path")
                    ...
                    .attr("d", () => {
                        const targetWidth = afterLink.target[0] == "Q" ? afterLink.height * REDUCTOR_Q : afterLink.target[0] == "K" ? afterLink.height * REDUCTOR_K : afterLink.height;
            ...
            //pegue os Vs cujo 
            const finalNodes = linksAfter.map(link => Vs._groups[0].filter(node => node.__data__.id == link.target))
